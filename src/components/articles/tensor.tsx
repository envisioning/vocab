...
import { ChevronsUp, ChevronsDown, Box, Info, Brain, Cubic, Grid3X3 } from "lucide-react";
...
export default function EnhancedTensorVisualizer() {
  const [dimension, setDimension] = useState<TensorDimension>(INITIAL_DIMENSION);
  ...
  const handleDimensionChange = (type: keyof TensorDimension, increment: boolean) => {
    setDimension((prev) => ({
      ...prev,
      [type]: increment ? Math.min(prev[type] + 1, 4) : Math.max(prev[type] - 1, 1),
    }));
  };
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-900 via-gray-800 to-gray-900 
      ...
      <div className="relative group mb-4">
        <p className="text-gray-300 text-center max-w-md">
          Tensors are multi-dimensional arrays used in neural networks to represent data and parameters
          <Info 
        ...
        {activeTooltip === "info" && (
          <div className="absolute -top-24 left-1/2 transform -translate-x-1/2 bg-gray-800 
                         p-4 rounded-lg shadow-xl border border-blue-500/30 w-64 text-sm">
            Think of tensors as containers that can hold numbers in multiple dimensions,
            like a 3D cube of data points essential for AI computations.
          </div>
        )}
      </div>
      ...
      <div className="flex gap-8 mb-8 p-4 bg-gray-800/50 rounded-xl backdrop-blur-sm">
        {[
          { label: "Width", key: "cols", icon: <Grid3X3 className="w-4 h-4" /> },
          { label: "Height", key: "rows", icon: <Grid3X3 className="w-4 h-4 rotate-90" /> },
          { label: "Depth", key: "depth", icon: <Cubic className="w-4 h-4" /> }
        ].map(({ label, key, icon }) => (
          <div key={key} className="flex flex-col items-center">
            ...
            <button
              onClick={() => handleDimensionChange(key as keyof TensorDimension, true)}
            ...
            <span className="py-2 font-mono text-lg text-blue-400">{dimension[key as keyof TensorDimension]}</span>
            <button
              onClick={() => handleDimensionChange(key as keyof TensorDimension, false)}
