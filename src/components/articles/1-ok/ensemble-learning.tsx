...
import { Users, Brain, VoteIcon, CheckCircle2, XCircle, RefreshCw } from "lucide-react"
...
const EnsembleLearningVisualizer = () => {
  ...
  const majorityVote = currentPredictionCase.modelPredictions.filter(p => p).length >
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-gray-50 rounded-lg shadow-md">
      <div className="flex justify-between items-center mb-8">
        {/* <h2 className="text-2xl font-bold text-gray-800">Ensemble Learning</h2> */}
        <div className="space-x-2">
          <button
            onClick={handleToggleAnimation}
            className="px-4 py-2 rounded bg-blue-500 text-white hover:bg-blue-600"
          >
            {state.isAnimating ? "Pause" : "Play"}
          </button>
          <button
            onClick={handleReset}
      ...
      <div className="mb-8 p-4 bg-white rounded-lg shadow">
        ...
        <div className="min-h-64 flex flex-col">
          <div className="flex justify-center space-x-4 mb-6">
            {currentPredictionCase.modelPredictions.map((prediction, idx) => (
              <div key={idx} className="flex flex-col items-center">
                <Brain
                  className={`w-12 h-12 mb-2 transition-colors duration-500 ${state.activeBrains[idx] ? "text-blue-500" : "text-gray-300"
                    }`}
                />
                <div className="relative h-10 w-10 flex items-center justify-center">
                  <div
                    className={`absolute transition-all duration-300 ${state.showVoting && state.activeBrains[idx] ?
                        "scale-100 translate-y-0 opacity-100" :
                        "scale-0 translate-y-0 opacity-0"
                      }`}
                  >
                    <div className={`p-2 rounded-full ${prediction ? "bg-green-100" : "bg-red-100"}`}>
                      {prediction ?
          ...
          <div className="mt-8 p-4 border-2 border-dashed border-gray-300 rounded-lg bg-gray-50">
            <div className={`flex flex-col items-center justify-center transition-all duration-500 ${state.showResult ? "opacity-100 transform-none" : "opacity-0 -translate-y-4"
              }`}>
              <VoteIcon className="w-8 h-8 text-blue-500 mb-2" />
              <div className="text-lg font-medium">
                Ensemble Prediction: {majorityVote ? "Yes" : "No"}
              </div>
              <div className={`mt-2 ${majorityVote === currentPredictionCase.correctAnswer ?
