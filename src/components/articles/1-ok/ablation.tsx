...
import { Music, Volume2, VolumeX, Brain, AlertCircle, ArrowRight } from "lucide-react";
...
const INSTRUMENTS: Instrument[] = [
    ...
    { id: 6, name: "Flute (Activation Functions)", importance: 0.6, active: true },
...
const AblationSimulator: React.FC<ComponentProps> = () => {
    ...
    const [phase, setPhase] = useState<'predict' | 'experiment' | 'conclusion'>('predict');

    const calculatePerformance = (activeInstruments: Instrument[]): number => {
        const totalImportance = INSTRUMENTS.reduce((sum, inst) => sum + inst.importance, 0);
    ...
    return (
        <div className="max-w-4xl mx-auto p-6 bg-gray-50 rounded-xl">
            <div className="mb-6 space-y-4">
                <h2 className="text-2xl font-bold text-gray-800">Neural Network Ablation Simulator</h2>
                <p className="text-gray-600">
                    {phase === 'predict' ? "Which component do you think is most critical?" :
                     phase === 'experiment' ? "Test your hypothesis by toggling components" :
                ...
                {instruments.map((instrument) => (
                    <button
                        ...
                        className={`p-4 rounded-lg flex flex-col items-center transition-all duration-300
                            ${instrument.active ? 'bg-blue-500 text-white' : 'bg-gray-300 text-gray-600'}
                            ${prediction === instrument.id ? 'ring-2 ring-green-500' : ''}`}
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm">
                ...
                {performance < 50 && (
                    <div className="mt-4 flex items-center text-red-500">
                        ...
                        <span>Critical components removed! Model performance severely impacted.</span>
                    </div>
                )}
            </div>
        </div>
