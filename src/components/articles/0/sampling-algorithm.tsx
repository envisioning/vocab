...
const SamplingVisualization = () => {
  ...
  const [sample, setSample] = useState<Point[]>([]);
  ...
  // Move generatePopulation outside useEffect
  ...
  // Update initial useEffect to use the function
  useEffect(() => {
  ...
  // Update handleReset to generate new population
  const handleReset = () => {
    setPopulation(generatePopulation());
    setSample([]);
    setIsPlaying(false);
