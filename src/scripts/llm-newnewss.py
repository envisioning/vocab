...
DATA_FILE = Path('../data/newness.json')
SCORE_FIELD = 'newness'
...
def calculate_newness_score(title: str, summary: str) -> list[float]:
    """Calculate newness scores using OpenAI API returning all 7 scores."""
    prompt = (
        "You are an AI expert. Please analyze the following AI/ML term and assign it 7 different newness scores "
        "from 0 to 1, where:\n"
        "- Scores near 1.0 are for concepts that are very new and emerging (e.g., 'Generative AI', 'Quantum Machine Learning')\n"
        "- Scores near 0.0 are for concepts that are well-established and older (e.g., 'Linear Regression', 'Decision Trees')\n"
        "Consider factors like: recent developments, current research focus, and emerging trends. Avoid responding 0.5 when you don't know - consider how this concept stacks against all other AI concepts in your knowledge.\n"
    ...
    try:
        logging.info("Sending request to OpenAI API for newness scoring")
        ...
        logging.info(f"Newness Scores: {scores}")
        return scores
    except KeyboardInterrupt:
        logging.info("\nScript interrupted by user. Shutting down gracefully...")
        sys.exit(0)
    except Exception as e:
        logging.error(f"Error during newness scoring: {e}")
...
def load_existing_scores():
    """Load existing scores from newnewss.json."""
    try:
        if DATA_FILE.exists():
        ...
        logging.error(f"Error loading newness.json: {e}")
        return {}

def save_scores(scores_dict):
    """Save scores to newness.json."""
    try:
        with DATA_FILE.open('w', encoding='utf-8') as f:
            json.dump(scores_dict, f, indent=2)
        logging.info("Saved scores to newness.json")
    except Exception as e:
        logging.error(f"Error saving newness.json: {e}")
...
def process_files(md_files):
    ...
    for md_file in md_files:
        ...
        if slug in existing_scores:
            logging.info(f"Skipping '{slug}' - already has newness scores")
            continue
        ...
        logging.info(f"Scoring newness for '{slug}'...")
        scores = calculate_newness_score(title, summary)
